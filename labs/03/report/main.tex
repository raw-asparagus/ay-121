...
\caption{2-D brute-force $\chi^2_\nu$ grid search in the $(b_{\mathrm{ew}}, b_{\mathrm{ns}})$ plane. \textit{Left:} Full-track search using all hour angles. \textit{Right:} Windowed search restricted to $|h| \leq 20^\circ$ around transit, suppressing Bessel-envelope bias. Solid and dashed contours mark $1\sigma$ confidence regions ($2.30\times$ and $6.17\times$ the minimum $\chi^2_\nu$). The windowed result ($b_{\mathrm{ew}} = 15.114$~m) is closer to the adopted baseline than the full-track grid ($b_{\mathrm{ew}} = 15.103$~m), consistent with a small systematic from the envelope modulation at large $|h|$.}
\label{fig:brute_2d}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_fringe_fit.pdf}
\caption{Recovered fringe pattern near transit ($|h| \leq 1.5^\circ$) from the windowed grid search. \textit{Top:} observed Re$(V)$ (scatter) and best-fit sinusoidal model (solid), illustrating the rapid fringe oscillation resolved by the interferometer. \textit{Bottom:} fit residuals with $\pm 3\sigma$ band. The close agreement validates the baseline solution and confirms that the point-source fringe model accurately describes the data at these hour angles.}
...
Table~\ref{tab:baseline_comparison} compares all five methods. The adopted baseline is the mean of the three methods included in the adoption set (STFT, NLS-complex, and delay-spectrum), with errors added in quadrature, yielding $b_{\mathrm{ew}} = 15.1148$~m and $b_{\mathrm{ns}} = 1.4746$~m. The formal uncertainties are sub-millimetre, reflecting the high SNR of the fringe signal across 285 spectral channels and $>$8000 time samples.
...
\midrule
\textbf{Adopted (mean)} & \textbf{15.1148} & \textbf{0.0000} & \textbf{1.4746} & \textbf{0.0000} & --- & --- \\
\midrule
...
\caption{Solar diameter from Bessel extrema, per channel. The horizontal dashed line shows the unweighted mean ($D_{\mathrm{mean}} = 34.664'$); the shaded band shows the $\pm 1\sigma$ statistical uncertainty. The diameter is consistent across the 70~MHz analysis band.}
