...
Figure~\ref{fig:lag_delay} shows the measured delay versus hour angle with the best-fit model overlaid. The sinusoidal sweep of $\tau(h)$ directly encodes all three key interferometric quantities: the amplitude of the sinusoidal variation gives $b_{\mathrm{ew}}$, the $\cos h$ modulation gives $b_{\mathrm{ns}}$, and the vertical offset (the fit intercept at $h = 0$ $\tau_{\mathrm{inst}} = 47.2$~ns) gives the effective cable delay $\tau'_c$; at transit ($h = 0$), the dominant EW term vanishes and the measured delay reads off $\tau'_c$ directly (see \S\ref{sec:cable_delay}).
...
Table~\ref{tab:baseline_comparison} compares all five methods. The adopted baseline is the mean of the three methods included in the adoption set (STFT, NLS-complex, and delay-spectrum), with per-method errors added in quadrature and the inter-method spread (sample standard deviation) added in quadrature as a systematic, yielding $b_{\mathrm{ew}} = 15.11 \pm 0.01$~m and $b_{\mathrm{ns}} = 1.49 \pm 0.05$~m. The formal uncertainties are sub-centimetre, reflecting the high SNR of the fringe signal across 285 spectral channels and $>$8000 time samples.

\begin{table*}[t]
\centering
\caption{Baseline comparison: all five methods. Columns show the east--west and north--south baseline components, their formal uncertainties, and the deviation from the adopted value in units of the formal error. The LiDAR measurement is shown for reference.}
...
\midrule
LiDAR measurement & 15.2 & 0.3 & 1.4 & 0.3 & $0.29$ & $-0.30$ \\
\bottomrule
\end{tabular}
\end{table*}

The adopted baseline agrees with the LiDAR measurement at the $0.3\sigma$ level (where $\sigma = 0.3$~m is the LiDAR uncertainty). The phase-slope method (Method~2) is a $>1\sigma$ outlier, consistent with the known phase-unwrapping bias from Bessel nulls.
...
\begin{equation}
\tau_{\mathrm{tot}}(h=0) = \underbrace{\frac{b_{\mathrm{ns}}}{c}\sin L\,\cos\delta}_{3.05~\mathrm{ns}} + \tau'_c.
\end{equation}
...
\[
D_{\mathrm{mean}} = 34.664 \pm 0.034\;\mathrm{arcmin}\quad\text{(statistical)}.
\]
...
We extend the uniform-disk model to a three-parameter fit: amplitude $A$, limb-brightening coefficient $\varepsilon$, and sunspot floor $f$, with $R$ fixed from the extrema prior. The limb-brightened visibility assumes a quadratic radial profile $I(\rho) \propto 1 + \varepsilon(\rho/R)^2$ \citep{Dulk1985}. Its Hankel transform decomposes into a uniform-disk (jinc) term and a quadratic correction:
\begin{equation}
V_{\mathrm{lb}}(x) = \frac{1}{1 + \varepsilon/2}\left[\frac{2\,J_1(x)}{x} + 2\varepsilon\int_0^1 t^3\,J_0(x\,t)\,\mathrm{d}t\right],
\end{equation}
where $x = 2\pi\,|u|\,R$. The first term is the standard jinc; the integral is precomputed via numerical quadrature on a lookup table and interpolated with a cubic spline. An unresolved active region adds a point-source floor $f$ in quadrature: $|V| = A\sqrt{V_{\mathrm{lb}}^2 + f^2}$.
...
\midrule
Extrema mean (this work) & $34.664 \pm 0.034$ & statistical only \\
...
The consistency of four out of five methods (all except the phase-slope outlier) at the sub-centimetre level demonstrates that a two-dish interferometer with $\sim$15~m baseline can determine its own geometry from the sky to far better precision than a hand-held LiDAR measurement.
