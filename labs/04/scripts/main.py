#!/usr/bin/env python3
"""Lab 4 - Follow-up observations for truncated spectra.

Two regimes based on which velocity end is truncated:
  - Redwards:  LO pair 1419/1420 MHz (extends positive-velocity coverage)
  - Bluewards: LO pair 1420/1421 MHz (extends negative-velocity coverage)

Cells are read from truncation_manifest.json (generated by
02c_truncation_flag.ipynb Section 10).  Completed cells are skipped
automatically.

Each regime runs as an independent session.  Sessions are written to
...
SURVEY_PARTS = [
    {
        'name': 'Follow-up redwards (LO 1419/1420)',
        'regime': 'redwards',
        'lo_on_mhz': 1419.0,
        'lo_off_mhz': 1420.0,
        'dumps_per_band': 4,
    ...
    {
        'name': 'Follow-up bluewards (LO 1420/1421)',
        'regime': 'bluewards',
        'lo_on_mhz': 1420.0,
        'lo_off_mhz': 1421.0,
        'dumps_per_band': 4,
...
TRUNCATION_MANIFEST_PATH = 'truncation_manifest.json'  # relative to labs/04/
...
# ---------------------------------------------------------------------------
# Cell loading from truncation manifest
# ---------------------------------------------------------------------------

def load_regime_cells(regime):
    """Load (l, b) cells for a regime from the truncation manifest.

    Returns cells as (row_idx, col_idx, l, b) tuples sorted in
    boustrophedon order (grouped by b, alternating l direction).
    """
    import sys
    from pathlib import Path

    script_dir = Path(__file__).resolve().parent.parent  # labs/04/
    manifest_path = script_dir / TRUNCATION_MANIFEST_PATH

    if not manifest_path.exists():
        print(f'  Truncation manifest not found at {manifest_path}')
        return []

    sys.path.insert(0, str(script_dir))
    from manifest import get_truncation_all_cells

    all_cells = get_truncation_all_cells(manifest_path, regime)
    if not all_cells:
        return []

    # Group by b, sort within each b row by l
    from collections import defaultdict
    by_b = defaultdict(list)
    for l, b in all_cells:
        by_b[b].append(l)

    cells = []
    for row_idx, b in enumerate(sorted(by_b)):
        l_vals = sorted(by_b[b])
        if row_idx % 2 == 0:
            l_vals = list(reversed(l_vals))  # boustrophedon
        for col_idx, l in enumerate(l_vals):
            cells.append((row_idx, col_idx, l, b))

    return cells


# ---------------------------------------------------------------------------
# Grid filtering (az-side and manifest)
# ---------------------------------------------------------------------------
...
def filter_cells_by_truncation_manifest(cells, regime):
    """Remove cells already complete in the truncation manifest."""
    import sys
    from pathlib import Path
    ...
    if not manifest_path.exists():
        print(f'  Truncation manifest not found --- keeping all cells')
        return cells

    sys.path.insert(0, str(script_dir))
    from manifest import get_truncation_complete_cells

    complete = get_truncation_complete_cells(manifest_path, regime)
    if not complete:
        print(f'  No complete cells in {regime} --- keeping all')
        return cells

    kept = [(r, c, l, b) for r, c, l, b in cells if (l, b) not in complete]
    n_skipped = len(cells) - len(kept)
    print(f'  Truncation manifest: {n_skipped} complete cells skipped, '
...
def main():
    print('Lab 4 - Follow-up for truncated spectra')
    print('=' * 60)

    for part_idx, part in enumerate(SURVEY_PARTS):
        regime = part['regime']
        lo_on_mhz = part['lo_on_mhz']
        lo_off_mhz = part['lo_off_mhz']
        dumps_per_band = part['dumps_per_band']
        dumps_per_cell = dumps_per_band * 2

        print(f'\n{"="*60}')
        print(f'  {part["name"]}')
        print(f'  Regime: {regime}, LO: {lo_on_mhz}/{lo_off_mhz} MHz')
        print(f'{"="*60}')

        # Load cells from truncation manifest, filter by az side and completeness
        all_cells = load_regime_cells(regime)
        if not all_cells:
            print('  No cells in manifest for this regime --- skipping.')
            continue

        cells = filter_cells_by_az_side(all_cells)
        cells = filter_cells_by_truncation_manifest(cells, regime)
        ...
        print(f'\nInitialising hardware (LO={lo_on_mhz} MHz) ...')
        telescope, sdrs, noise = setup_hardware(lo_on_mhz)
    ...
    print('\n' + '=' * 60)
    print('  All follow-up parts complete!')
    print('=' * 60)
