...
Grid: two-phase brick-pattern tessellation, column-major scan order.
      ...
      Odd phase:  b=[-5, -3, -1, +1, +3, +5], l offset by half a physical step.
...
OUTPUT_DIR   = 'data/main'  # relative to cwd (expected: labs/04/)
...
def _load_reobserve_set():
    ...
    reobs_path = Path('artifacts/main_reobserve.json')
...
def _bright_body_radec(unix_t):
    ...
    the parallax is small (<1 deg) and irrelevant at the 10 deg avoidance
    threshold.
    """
